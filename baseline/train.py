...
def generate_input(parameters, mode, loc_old2newid, user_topk=None, mode2=None, candidate=None):
	...
	for u in candidate:
		...
		for c, i in enumerate(train_id): #day
			...
			ptim_np = np.reshape(np.array([s[0] for s in session[4:]]), (len(session[:-4]), 1))
			...
			#app_np = app_np*2 -1 #re-scale to -1~1
			loc_target = np.array([loc_old2newid[s[1]] for s in session[4:]])
			loc_loss =  np.array([loc_old2newid[s[1]] for s in session[:-4]])
			#loc_target = np.array([s[1] for s in session[1:]])
			app_target = np.reshape(np.array([s[2] for s in session[4:]]), (len(session[:-4]), parameters.app_size))
...
def run_simple(data, run_idx, mode, inputmode, lr, clip, model, optimizer, criterion, mode2=None,test_start=3, threshold=0.3):
	...
	for c in range(queue_len):
		...
		elif mode2 in ['AppPreUserIdenGtr']:
